...
#pragma omp parallel private(p,n,jc,kc,index,count,indexreal,indexhalo)
  {
    nthreads = omp_get_num_threads();
    // this is threadprivate
...
#pragma omp for
    ...
    /* Wait until everything has been packed by all threads */
#pragma omp barrier

    if (cart_size(X) == 1) {

      /* Master thread copies buffers */
#pragma omp master
      {
      ...
      int sizeid  = nsend/nthreads;
      int startid = sizeid*threadid;


      MPI_Irecv(&recvforw[startid], sizeid, MPI_DOUBLE, pforw, tagb+threadid, comm, req);
      MPI_Irecv(&recvback[startid], sizeid, MPI_DOUBLE, pback, tagf+threadid, comm, req + 1);


      MPI_Issend(&sendback[startid], sizeid, MPI_DOUBLE, pback, tagb+threadid, comm, req + 2);
      MPI_Issend(&sendforw[startid], sizeid, MPI_DOUBLE, pforw, tagf+threadid, comm, req + 3);
...
#pragma omp master
  ...
  } //end of parallel region X-direction
...
#pragma omp parallel private(p,n,ic,kc,index,count,indexreal,indexhalo)
  {
    nthreads = omp_get_num_threads();
    // this is threadprivate
    int threadid = omp_get_thread_num();
...
#pragma omp barrier
    ...
    //#pragma omp parallel for private(p,n,ic,kc,index,count,indexhalo)
#pragma omp for    
  for (p = 0; p < NVEL; p++)
    for (n = 0; n < lb->ndist; n++)
      for (ic = 0; ic <= nlocal[X] + 1; ic++)
...
#pragma omp master
 ...
 }//end of parallel region Y-direction
  ...
  count = 0;
#pragma omp parallel private(p,n,ic,jc,index,count,indexreal,indexhalo)
  {
    nthreads = omp_get_num_threads();
    // this is threadprivate
    int threadid = omp_get_thread_num();
...
#pragma omp master
    ...
    /* This barrier seems to be needed to guaranted correctness */
#pragma omp barrier
...
#pragma omp master
  ...
  } //end parallel region Z-direction
